...
from tabdml.parallel import build_stage2_resume_worker_commands, run_workers
...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        default="configs/stage2_parallel/ensemble_05.yaml",
    )
    parser.add_argument("--output-root", default="results/raw")
    parser.add_argument("--log-dir", default="results/logs/stage2_resume_8")
...
def main() -> int:
    ...
    print(f"Launching {len(commands)} Stage 2 resume workers.", flush=True)
    ...
    if failed:
        ...
        return 1
    print("All Stage 2 resume workers completed.", flush=True)
    return 0
